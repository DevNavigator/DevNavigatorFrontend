...
const Study: React.FC = () => {
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-4 md:p-6">
      {loading && <p className="text-center text-lg text-blue-500">Cargando cursos...</p>}
      {error && <p className="text-center text-lg text-red-500">{error}</p>}
      {!loading && !error && courses.length === 0 && (
        <p className="text-center text-lg text-gray-700">No hay cursos disponibles.</p>
      )}
      {!loading && !error && courses.length > 0 && (
        <div className="container mx-auto p-6 bg-white shadow-lg rounded-lg">
          <h2 className="text-3xl font-bold text-center text-blue-600 mb-6">Lista de Cursos</h2>
          <ul className="space-y-4">
            {courses.map((course) => (
              <li key={course.id} className="bg-gray-100 p-4 rounded-md shadow-sm hover:shadow-md transition duration-300">
                ...
                <div className="mt-6">
                  ...
                  <ul className="space-y-2 mt-2">
                    {course.content.map((video, index) => (
                      <li
                        key={index}
                        className={`cursor-pointer text-blue-500 hover:underline ${videoCompleted[index] ? "line-through text-gray-400" : ""}`}
                        onClick={() => {
                          setActiveVideo(index);
                          setShowQuestions(false);
                          setQuizCompleted(false);
