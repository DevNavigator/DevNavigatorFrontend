...
const CourseDetail: React.FC = () => {
  ...
  const handleSubmit = () => {
    ...
    course.questions.forEach((question, index) => {
      ...
      if (userAnswer === correctAnswer) {
        ...
        feedbackObj[index] = `Incorrecto. La respuesta correcta es: ${correctAnswer}`;
      }
    });
  ...
  return (
    <div className="p-6">
      ...
      <img src={course.image_url} alt={course.title} className="rounded-lg mb-2" />
      ...
      {Object.keys(feedback).length > 0 && (
        <div className="mt-4">
          {Object.entries(feedback).map(([index, message]) => (
            <div key={index} className="text-sm text-gray-600">{message}</div>
          ))}
        </div>
      )}
    </div>
